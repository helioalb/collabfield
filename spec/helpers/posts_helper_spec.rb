...
RSpec.describe PostsHelper, type: :helper do
  context '#create_new_post_partial_path' do
    it "returns a signed_in partial's path" do
      ...
      expected_path = 'posts/branch/create_new_post/signed_in'
      expect(helper.create_new_post_partial_path).to eq expected_path
    end

    it "returns a not signed_in partial's path" do
      allow(helper).to receive(:user_signed_in?).and_return(false)
      expected_path = 'posts/branch/create_new_post/not_signed_in'
      expect(helper.create_new_post_partial_path).to eq expected_path
    end
  end
  ...
  context '#all_categories_button_partial_path' do
    it "returns an all_selected partial's path" do
      ...
      expected_path = 'posts/branch/categories/all_selected'
      expect(helper.all_categories_button_partial_path).to eq expected_path
    end

    it "returns an all_not_selected partial's path" do
      controller.params[:category] = 'category'
      expected_path = 'posts/branch/categories/all_not_selected'
      expect(helper.all_categories_button_partial_path).to eq expected_path
    end
  end
